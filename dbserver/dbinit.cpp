...
U32 dbLockIdFromLink(NetLink *link)
{
	...
	if (!link)
		return 0;
	client = (DBClientLink*) link->userData;
...
NetLink *dbLinkFromLockId(int lock_id)
{
	...
	if ( lock_id && stashIntFindPointer(lock_id_hashes, lock_id, (void**) &link))
		return link;
	return NULL;
}
...
{
	DBClientLink	*client = (DBClientLink*) link->userData;
	MapCon			*container;
	int				i;
	...
	if (client->map_id < 0)
		return 1;
	container = (MapCon*) containerPtr(map_list,client->map_id);
...
void getStaticMapList()
{
	...
	dynArrayAdd((void**) &static_map_ids,sizeof(static_map_ids[0]),&static_map_count,&static_map_max,1);
	for(i=0;i<maps->num_alloced;i++)
	{
		mapcon = (MapCon*) maps->containers[i];
		if (mapcon->is_static)
		{
			idp = (int*) dynArrayAdd((void** )&static_map_ids,sizeof(static_map_ids[0]),&static_map_count,&static_map_max,1);
			*idp = mapcon->id;
		}
	}
}
...
int registerDbClient(Packet *pak,NetLink *link)
{
	...
	if (map_id < 0) // mainly used by mapservers running in 'localmapserver' (editing) mode
	{
		...
		if (!container)
		{
			container = (MapCon*) containerAlloc(map_list,-1);
	...
	{
		container = (MapCon*) containerPtr(map_list,map_id);
	}
	if (!container)
		return 0;
...
static void detachContainerServer(DBClientLink* client, DbList* dblist)
{
	...
	for (i=0; i<net_links.links->size; i++) 
	{
		...
		link = (NetLink*) net_links.links->storage[i];
		waitclient = (DBClientLink*) link->userData;
...
void containerServerNotify(int list_id, U32 cid, U32 dbid, int add)
{
	...
	{
		EntCon* entcon = (EntCon*) containerPtr(dbListPtr(list_id), cid);
		if (entcon)
		{
			if (entcon->teamup_id)
...
extern "C" NetLinkList launcher_links;
extern "C" NetLinkList svrMonLinks;
extern "C" NetLinkList gameclient_links;
extern "C" int crashmap_sock;
...
static void sendAuthUserData(int container_id,void *data_void)
{
	...
	int		binlen;
	EntCon	*c = (EntCon*) containerPtr(ent_list,container_id);
...
static void accountCacheSetSize(int container_id, void *data_void)
{
	...
	ShardAccountCon	*c = (ShardAccountCon*) containerPtr(shardaccounts_list, container_id);
...
void dbInit(int start_static)
{
	...
	case (DBPROV_MSSQL):
		gContainerDb = new ContainerDbMssql();
		gDatabaseProvider = DBPROV_MSSQL;
		break;
	case (DBPROV_POSTGRESQL):
		gContainerDb = new ContainerDbPostgresql();
		gDatabaseProvider = DBPROV_POSTGRESQL;
		break;
	default:
		FatalErrorf("No implementation for SqlDbProvider %d found", gDatabaseProvider);
	}
	...
	printf_stderr("%s SQL server connected\n", timerGetTimeString());
	sqlFifoInit();
	...
	ent_list = containerListRegister(CONTAINER_ENTS,"Ents",sizeof(EntCon), (StatusCallback)entStatusCb,entContainerUpdtCb,ent_cmds);
	doors_list = containerListRegister(CONTAINER_DOORS,"Doors",sizeof(DoorCon), (StatusCallback)doorStatusCb,0,0);
	teamups_list = containerListRegister(CONTAINER_TEAMUPS,"Teamups",sizeof(GroupCon), (StatusCallback)groupStatusCb,0,team_cmds);
	supergroups_list = containerListRegister(CONTAINER_SUPERGROUPS,"Supergroups",sizeof(GroupCon), (StatusCallback)groupStatusCb,0,sgroup_cmds);
	taskforces_list = containerListRegister(CONTAINER_TASKFORCES,"Taskforces",sizeof(GroupCon), (StatusCallback)groupStatusCb,0,team_cmds);
	...
	crashedmap_list = containerListRegister(CONTAINER_CRASHEDMAPS,"CrashedMaps",sizeof(MapCon), (StatusCallback)mapStatusCb,0,0);
	mapgroups_list = containerListRegister(CONTAINER_MAPGROUPS,"MapGroups",sizeof(GroupCon), (StatusCallback)groupStatusCb,0,0);
	map_list = containerListRegister(CONTAINER_MAPS,"maps",sizeof(MapCon), (StatusCallback)mapStatusCb,0,map_cmds);
	...
	launcher_list = containerListRegister(CONTAINER_LAUNCHERS,"launchers",sizeof(LauncherCon), (StatusCallback)launcherStatusCb,0,0);
	serverapp_list = containerListRegister(CONTAINER_SERVERAPPS,"ServerApps",sizeof(ServerAppCon), (StatusCallback)serverAppStatusCb,0,0);
	mineacc_list = containerListRegister(CONTAINER_MININGACCUMULATOR,"MiningAccumulator",sizeof(DbContainer),0,0,0);
	levelingpacts_list = containerListRegister(CONTAINER_LEVELINGPACTS,"LevelingPacts",sizeof(GroupCon), (StatusCallback)groupStatusCb,0,0);
	league_list = containerListRegister(CONTAINER_LEAGUES,"Leagues",sizeof(GroupCon),(StatusCallback)groupStatusCb,0,0);
	eventhistory_list = containerListRegister(CONTAINER_EVENTHISTORY,"EventHistory",sizeof(DbContainer), (StatusCallback)dbEventHistory_StatusCb,(ContainerUpdateCallback)dbEventHistory_UpdateCb,0);
...
#ifndef FINAL
extern "C" int g_force_production_mode;
#endif
