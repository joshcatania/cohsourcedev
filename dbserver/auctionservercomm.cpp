...
NetLink *auctionLink(bool bThrottle)
{
	...
	return (NetLink*) eaGet(&auction_state.links,auction_state.link_rotor);
}
...
#define AUC_SVR_RECV() \
			int ident = pktGetBitsAuto(pak_in); \
			EntCon *ent_con = (EntCon*) containerPtr(ent_list, ident);   \
			NetLink *map_link = (NetLink*) ent_con?dbLinkFromLockId(ent_con->lock_id):NULL;
...
static int auctionMsgCallback(Packet *pak_in,int cmd,NetLink *auc_link)
{
	...
	case AUCTION_SVR_CONNECT:
	{
		...
		if(!auction_state.sqlcb_link && eaSize(&auction_state.links) == 1)
		{
			...
			std::string whereClause = "WHERE LastActive > " + gContainerDb->nowMinusDays(days_active);

			sqlReadColumnsAsync(ent_list->tplt->tables,0,"ContainerId", whereClause.c_str(),sqlAuctionGetActiveCallback,0,pak_out,0);
	...
	case AUCTION_SVR_BATCH_SEND_ITEMINFO:
	{
		...
		for(i=0;i<map_list->num_alloced;i++)
		{
			...
			{
				DBClientLink *client = (DBClientLink*) map->link->userData;
	...
	case AUCTION_SVR_XACT_CMD:
	{
		...
		type = (XactUpdateType) pktGetBitsAuto(pak_in);
...
static int auction_clientDeleteCb(NetLink *link)
{
	...
	if(*type == kAuctionServerType_Heroes)
	{
		// The casts for eaFindAndRemove don't compile in C++ so they've been separated out and we use a new function
		// which doesn't do the casts itself.
		eaFindAndRemoveCpp(const_cast<cEArrayHandle*>(reinterpret_cast<mEArrayHandle*>(&auction_state.links)), link);
