...
extern "C" {
	extern void teamCheckLeftMission(int map_id, int team_id, char* sWhy);
	extern bool checkContainerBadges_Internal(const char* container);
}

int compareLines(const void* lhs, const void* rhs) {
	return ((LineTracker*)lhs)->idx - ((LineTracker*)rhs)->idx;
}
...
char *findFieldTplt(ContainerTemplate *tplt,LineList *list,char *field,char *buf)
{
	...
	if (!stashFindInt(tplt->all_hashes,field,&idx))
		return 0;
	search.idx = idx;
	line = (LineTracker*) bsearch(&search,list->lines,list->count,sizeof(search),compareLines);
	if (line)
...
{
	DbList	*list;
	DbContainer	*container = (DbContainer*) any_container;
...
static void containerFree(AnyContainer *container_void)
{
	int		i;
	DbList	*list;
	DbContainer	*container = (DbContainer*) container_void;
	...
	case CONTAINER_ENTS:
		{
			...
			DbContainer *ent_con;
			if(stashIntFindPointer(list->other_hashes, auth_id, (void**) &ent_con) && ent_con == container)
			{
	...
	case CONTAINER_SHARDACCOUNT:
		{
			...
			if(stashFindPointer(list->other_hashes,((ShardAccountCon*)container)->account,(void**) &shardaccount_con) && shardaccount_con == container)
			{
	...
	{
		GroupCon	*group = (GroupCon*) container_void;
								...
								ContainerUpdateCallback updt_cb, SpecialColumn *special_columns )
{
	...
	if (list->id == CONTAINER_ENTS)
		...
		list->other_hashes = stashTableCreateWithStringKeys(0, StashDefault);
	return list;
}
...
void containerGroupGetMembership(DbList *list, int id)
{
	...
	if(!list->tplt->member_table) // members are run-time objects,
		return;
	group = (GroupCon*) containerPtr(list, id);
	if(group && !group->member_id_count)
	{
		...
		if(group->type == CONTAINER_SUPERGROUPS)
		{
				...
				atoi(buf) > 0 )
			{
				...
				base = (DbContainer*) containerLoad(dbListPtr(CONTAINER_BASE), base_id);
...
static void setSpecialColumns(DbList *list,DbContainer *container,char *data,LineList *line_list,char **cache_data)
{
	...
	for(i=0;cmds[i].name;i++)
	{
		...
		while (true)
		{
			...
			char * dest = (cmds[i].cmd != CMD_ARRAY && (cmds[i].type == CFTYPE_STRING || cmds[i].type == CFTYPE_WSTRING)) ? (char*)((U8*) container + cmds[i].offset) : buf;
			char * name = NULL;
...
AnyContainer *containerUpdate(DbList *list,int id,char *diff_orig,int update_sql)
{
	...
	if (!list)
		return 0;
	container = (DbContainer*) containerPtr(list,id);
	if (!list->tplt)
	{
		if (!container)
			return 0;
		tpltUpdateData(&container->raw_data,(int*) &container->container_data_size,diff_orig,list->tplt);
...
AnyContainer *containerUpdate_notdiff(DbList *list,int id,char *str)
{
	...
	if (id < 0)
		container = (DbContainer*) containerAlloc(list,id);
	else
		container = (DbContainer*) containerPtr(list,id);
	if (!container)
		return 0;
...
{
	DbContainer			*container = (DbContainer*) containerPtr(list,id);
	int					save_ok = 1;
...
AnyContainer *containerPtr(DbList *list,int idx)
{
	...
	if (!list || idx < 0)
		return 0;
	if (!idx || !stashIntFindPointer(list->cid_hashes, idx, (void**) &c))
		c = NULL;
	assert(!c || c->id == idx);
...
EntCon *containerFindByAuthId(U32 auth_id)
{
	...
	if(!auth_id || !stashIntFindPointer(dbListPtr(CONTAINER_ENTS)->other_hashes, auth_id, (void**) &ent_con))
		ent_con = NULL;
...
ShardAccountCon* containerFindByAuthShardAccount(const char *auth)
{
	...
	if(!auth || !stashFindPointer(dbListPtr(CONTAINER_SHARDACCOUNT)->other_hashes,auth,(void**) &shardaccount_con))
...
AnyContainer *containerAlloc(DbList *list,int idx)
{
	...
	if (idx < 0) // get any open slot
	{
		...
		if (list->tplt)
		{
			...
			{
				std::string query = gContainerDb->nextInsertedSequenceValueQuery(list->tplt->tables->name, "containerid");
				list->max_container_id = sqlGetSingleValue(query.c_str(), SQL_NTS, NULL, SQLCONN_FOREGROUND);
			}
	...
	if ((container = (DbContainer*) containerPtr(list,idx)))
		return container;

	container = (DbContainer*) calloc(list->struct_size,1);
	container->on_since = timerSecondsSince2000() - 1;
	container->type = list->id;
	container->id  = idx;
	container->just_created = make_new;
	dynArrayAddp((void***) &list->containers,&list->num_alloced,&list->max_alloced,container);
...
void containerSendList(DbList *list,int *selections,int count,int lock,NetLink *link,U32 user_data)
{
	...
	for(i=0;i<count;i++)
	{
		...
		container = (DbContainer*) containerPtr(list,selections[i]);
...
void containerSendStatus(DbList *list,int *selections,int count,NetLink *link,U32 user_data)
{
	...
	for(i=0;i<count;i++)
	{
		con = (DbContainer*) containerPtr(list,selections[i]);
		if (!con ||
...
int *listGetAll(DbList *list,int *ent_count,int active_only)
{
	...
	for(*ent_count=i=0;i<list->num_alloced;i++)
	{
		if (list->containers[i] && list->containers[i]->active >= (U32)active_only) {
			int *p = (int*) dynArrayAdd((void**) &elist, sizeof(int), ent_count, &maxcount, 1);
...
int containerIdFindByElement(DbList *list, char *element, char *value)
{
	...
	ZeroArray(bind_list);//
	bind_list[0].data_type = CFTYPE_STRING;
...
void containerSetMemberState(DbList *list,int id,int member_id,int loaded)
{
	...
	group_con = (GroupCon*) containerPtr(list,id);
	if (!group_con)
		return;
...
void containerAddMember(DbList *list, int id, int member_id, int flags)
{
	...
	GroupCon *container = (GroupCon*) containerPtr(list,id);
	if (!container)
		return;
	...
	dynArrayAdd((void**) &container->member_ids,sizeof(container->member_ids[0]),&container->member_id_count,&container->member_id_max,1);
	if (num_alloced != container->member_id_max)
		container->member_ids_loaded = (U32*) realloc(container->member_ids_loaded,4 * ((container->member_id_max + 31)/32));
...
static void sendLostMembership(int list_id, int container_id, int ent_id)
{
	...
	ent_con = (EntCon*) containerPtr(ent_list,ent_id);
	if(!ent_con)
		return;
