...
void tpltRegisterForeignKeyConstraint(char *table, char *key, char *foreign_table)
{
	...
	fk = (ForeignKey*) dynArrayAdd((void**) &foreign_keys,sizeof(foreign_keys[0]),&foreign_key_count,&foreign_key_max,1);
...
void hashAllNames(ContainerTemplate *tplt, const char * fname)
{
			...
			for(j=0;j<table->num_columns;j++)
			{
				...
				if (count >= max_slots)
				{
					if (!max_slots)
						max_slots = 100;
					max_slots *= 2;
					tplt->slots = (SlotInfo*) realloc(tplt->slots,sizeof(SlotInfo) * max_slots);
				}
	...
	tplt->lines = (char**) calloc(count,sizeof(tplt->lines[0]));
}
...
ContainerTemplate *tpltLoad(int dblist_id,char *table_name,char *fname)
{
	...
	mem = (char*) fileAlloc(fname,0);
	if (!mem)
		return NULL;

	tplt = (ContainerTemplate*) calloc(sizeof(ContainerTemplate),1);
	...
	while(ctnrLineGet(&state))
	{
		...
		{
			tplt->table_count++;
			tplt->tables = (TableInfo*) realloc(tplt->tables,tplt->table_count * sizeof(tplt->tables[0]));
			...
			// SubId or Active
			field = table->columns = (ColumnInfo*) realloc(field,2 * sizeof(*field));
		...
		if (1 || state.table_idx <= 0)
		{
			...
			// SQL_GD_ANY_COLUMN. All columns are presorted to be bound to unbound anyway.
			...
			table->num_columns++;
			table->columns = (ColumnInfo*) realloc(table->columns,table->num_columns * sizeof(table->columns[0]));
...
static bool verifyTableColumns(TableInfo *table)
{
	...
	if (eaiSize(&rebuild_columns))
	{
		...
		while (eaiSize(&rebuild_columns))
		{
			...
			printf("Table: %s, field:%s, field data type: %s, new field data type:%s\n", table->name, field->name, field->data_type_name, new_field->data_type_name);
...
static bool rebuildTableDataInPlace(ContainerTemplate *tplt, TableInfo *table)
{
	...
	std::string firstQuery = gContainerDb->truncateTempTableAndInsertInto(table->name, columns, table->table_type == TT_CONTAINER);

	// Copy to temporary table
	ret = sqlExecDdlf(DDL_REBUILDTABLE, firstQuery.c_str());
	...
	if (ret)
	{
		...
		std::string secondQuery = gContainerDb->insertIntoTableFromTemp(table->name, columns, table->table_type == TT_CONTAINER);

		// Copy to final location
		ret = sqlExecDdlf(DDL_REBUILDTABLE, secondQuery.c_str());
	}
...
static bool loadAttributesFromFile(const char * fname, AttributeList * attr)
{
	int count;
	char *args[2];
	char *mem = (char*) fileAlloc(fname,0);
	char *s = mem;
	...
	while((count=tokenize_line(s,args,&s)))
	{
		...
		if(strHasSQLEscapeChars((unsigned char*) args[1]))
		{
...
{
	int attr_id = 0;
	AttributeList *attr = (AttributeList*) calloc(1, sizeof(*attr));
...
void tpltDeleteAll(ContainerTemplate *tplt, char *null_field, bool renumber)
{
	...
	for(i=tplt->table_count-1; i>0; i--)
	{
		...
		std::string query = gContainerDb->deleteFromContainer(sub, master, (null_field) ? null_field : "");
	...
	{
		sqlFifoBarrier();
		std::string restartSequence = gContainerDb->restartContainerSequence(master);
		sqlExecAsync(restartSequence.c_str(), SQL_NTS);
	}
