...
struct asql_inventory_bytes {
	SQLINTEGER sku_id_bytes;
	SQLINTEGER granted_total_bytes;
	SQLINTEGER claimed_total_bytes;
	SQLINTEGER saved_total_bytes;
	SQLINTEGER expires_bytes;
};

struct asql_game_transaction_bytes {
	SQLINTEGER order_id_bytes;
	SQLINTEGER auth_id_bytes;
	SQLINTEGER sku_id_bytes;
	SQLINTEGER transaction_date_bytes;
	SQLINTEGER shard_id_bytes;
	SQLINTEGER ent_id_bytes;
	SQLINTEGER granted_bytes;
	SQLINTEGER claimed_bytes;
	SQLINTEGER csr_did_it_bytes;
};
...
void asql_sync_product_types() {
	...
	SQLINTEGER name_bytes[kAccountInventoryType_Count];
...
void asql_sync_products() {
	...
	SQLINTEGER* sku_bytes = new SQLINTEGER[size];

	char (*names)[128] = new char[size][128];
	SQLINTEGER* name_bytes = new SQLINTEGER[size];

	long * product_type_ids = new long[size];

	long * grant_limits = new long[size];
	SQLINTEGER* grant_limit_bytes = new SQLINTEGER[size];

	long * expiration_seconds = new long[size];
	SQLINTEGER* expiration_second_bytes = new SQLINTEGER[size];
...
bool asql_search_for_account(SqlConn conn, const char *auth_name_or_id, U32 *found_auth_id)
{
	...
	SQLINTEGER found_auth_id_bytes = sizeof(*found_auth_id);
	...
	SQLINTEGER search_for_id_bytes = search_for_id ? sizeof(search_for_id) : SQL_NULL_DATA;
	
	SQLINTEGER auth_name_or_id_bytes = SQL_NTS;
...
static bool asql_find_or_create_account(SqlConn conn, asql_account * acc, asql_inventory ** inv_list, int * inv_count) {
	...
	*inv_list = NULL;
	...
	SQLINTEGER name_size = acc->name[0] ? strlen(acc->name) : SQL_NULL_DATA;
	SQLINTEGER loyalty_bits_size = LOYALTY_SQL_BYTES;
	SQLINTEGER last_email_date_size = sizeof(acc->last_email_date);
	SQLINTEGER free_xfer_date_size = sizeof(acc->free_xfer_date);
...
static bool asql_update_account(SqlConn conn, asql_account * acc) {
	SQLINTEGER loyalty_bits_size = LOYALTY_SQL_BYTES;
	SQLINTEGER last_email_date_is_null = SQL_NULL_DATA;
	SQLINTEGER free_xfer_date_is_null = SQL_NULL_DATA;
...
static bool asql_add_micro_transaction(SqlConn conn, asql_micro_transaction * mtx, asql_inventory * inv) {
	SQLINTEGER sku_bytes = sizeof(mtx->sku_id);
...
static bool asql_add_game_transaction(SqlConn conn, asql_game_transaction * gtx, asql_inventory * inv) {
	SQLINTEGER sku_bytes = sizeof(gtx->sku_id);
	SQLINTEGER shard_id_is_null = SQL_NULL_DATA;
	SQLINTEGER ent_id_is_null = SQL_NULL_DATA;
	SQLINTEGER granted_is_null = SQL_NULL_DATA;
	SQLINTEGER claimed_is_null = SQL_NULL_DATA;
...
static bool asql_add_multi_game_transaction(SqlConn conn, MultiGameTransaction *transaction) {
	...
	SQLINTEGER sku_bytes[MAX_MULTI_GAME_TRANSACTIONS];

	SQL_TIMESTAMP_STRUCT transaction_dates[MAX_MULTI_GAME_TRANSACTIONS];

	U8 shard_ids[MAX_MULTI_GAME_TRANSACTIONS];
	SQLINTEGER shard_bytes[MAX_MULTI_GAME_TRANSACTIONS];

	U32 ent_ids[MAX_MULTI_GAME_TRANSACTIONS];
	SQLINTEGER ent_bytes[MAX_MULTI_GAME_TRANSACTIONS];

	long granted_values[MAX_MULTI_GAME_TRANSACTIONS];
	SQLINTEGER granted_bytes[MAX_MULTI_GAME_TRANSACTIONS];

	long claimed_values[MAX_MULTI_GAME_TRANSACTIONS];
	SQLINTEGER claimed_bytes[MAX_MULTI_GAME_TRANSACTIONS];
