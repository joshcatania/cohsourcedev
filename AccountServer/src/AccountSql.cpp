...
struct asql_inventory_bytes {
	long sku_id_bytes;
	long granted_total_bytes;
	long claimed_total_bytes;
	long saved_total_bytes;
	long expires_bytes;
};
...
struct asql_game_transaction_bytes {
    ...
    long auth_id_bytes;
    long sku_id_bytes;
    long transaction_date_bytes;
    long shard_id_bytes;
    long ent_id_bytes;
    long granted_bytes;
    long claimed_bytes;
    long csr_did_it_bytes;
};
...
void asql_sync_product_types() {
    ...
    long name_bytes[kAccountInventoryType_Count];
...
void asql_sync_products() {
	...
	long* sku_bytes = new long[size];

    char (*names)[128] = new char[size][128];
	long* name_bytes = new long[size];

    long * product_type_ids = new long[size];

    long * grant_limits = new long[size];
	long* grant_limit_bytes = new long[size];

    long * expiration_seconds = new long[size];
	long* expiration_second_bytes = new long[size];
...
bool asql_search_for_account(SqlConn conn, const char *auth_name_or_id, U32 *found_auth_id)
{
	...
	long found_auth_id_bytes = sizeof(*found_auth_id);
	...
	long search_for_id_bytes = search_for_id ? sizeof(search_for_id) : SQL_NULL_DATA;
    
	long auth_name_or_id_bytes = SQL_NTS;
...
static bool asql_find_or_create_account(SqlConn conn, asql_account * acc, asql_inventory ** inv_list, int * inv_count) {
    ...
    *inv_list = NULL;
    ...
    long name_size = acc->name[0] ? strlen(acc->name) : SQL_NULL_DATA;
    long loyalty_bits_size = LOYALTY_SQL_BYTES;
    long last_email_date_size = sizeof(acc->last_email_date);
    long free_xfer_date_size = sizeof(acc->free_xfer_date);
...
static bool asql_update_account(SqlConn conn, asql_account * acc) {
    long loyalty_bits_size = LOYALTY_SQL_BYTES;
    long last_email_date_is_null = SQL_NULL_DATA;
    long free_xfer_date_is_null = SQL_NULL_DATA;
...
static bool asql_add_micro_transaction(SqlConn conn, asql_micro_transaction * mtx, asql_inventory * inv) {
	long sku_bytes = sizeof(mtx->sku_id);
...
static bool asql_add_game_transaction(SqlConn conn, asql_game_transaction * gtx, asql_inventory * inv) {
    long sku_bytes = sizeof(gtx->sku_id);
    long shard_id_is_null = SQL_NULL_DATA;
    long ent_id_is_null = SQL_NULL_DATA;
    long granted_is_null = SQL_NULL_DATA;
    long claimed_is_null = SQL_NULL_DATA;
    long saved_is_null = SQL_NULL_DATA;
