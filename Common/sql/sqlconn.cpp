...
typedef std::atomic<hrclock::duration::rep> AtomicDuration;

typedef __declspec(align(EXPECTED_WORST_CACHE_LINE_SIZE)) struct {
	volatile AtomicDuration worst_usec;
	volatile AtomicDuration running_usec;
	volatile AtomicDuration idle_usec;
	volatile AtomicDuration total_count;
} sqlTimes;

struct sqlGlobals {
    U32 connection_count;
    sqlTimes times[SQLCONN_MAX];
	timer timers[SQLCONN_MAX];
...
static const char* sqlTypeDataString(SQLSMALLINT type, SQLSMALLINT ctype, SQLPOINTER data, SQLLEN length)
{
    THREADSAFE_STATIC char *buf = NULL;
    THREADSAFE_STATIC_MARK(buf);
    ...
    switch (ctype) {
        ...
        {
            if (length != SQL_NTS)
                estrConcatFixedWidth(&buf, cpp_reinterpret_cast(const char*)(data), length);
            else
                estrConcatCharString(&buf, cpp_reinterpret_cast(const char*)(data));
        }
        xcase SQL_WCHAR:
        case SQL_WVARCHAR:
            estrConvertToNarrow(&buf, cpp_reinterpret_cast(wchar_t*)(data));
        xcase SQL_DATETIME:
        {
            SQL_TIMESTAMP_STRUCT *t = cpp_reinterpret_cast(SQL_TIMESTAMP_STRUCT*)(data);
...
static bool sqlConnPrintError(SQLSMALLINT handle_type, SQLHANDLE handle, const char *original_command, char *sql_state_out)
{
    ...
    while ((ret = SQLGetDiagRecA(handle_type, handle, rec_num, sql_state, &error, msg, sizeof(msg), &msg_len)) != SQL_NO_DATA)
    {
        ...
        assertmsg(strcmp((const char*)sql_state, "08007")!=0, "Connection failure during transaction");
        assertmsg(strcmp((const char*)sql_state, "08S01")!=0, "Communication link failure");
        assertmsg(strcmp((const char*)sql_state, "HYT01")!=0, "Connection timeout expired");
...
static bool sqlConnPrintErrorf(SQLSMALLINT handle_type, SQLHANDLE handle, const char *fmt, ...)
{
    char buf[4096];
    va_list va;
    bool ret;
...
void sqlConnGetTimes(int * count, long * avg_usec, long * worst_usec, float * fore_idle_ratio, float * back_idle_ratio)
{
    ...
    for(i=0; i<sql.connection_count; i++) {
		...
		current.worst_usec.exchange(0);
		current.running_usec.exchange(0);
		current.idle_usec.exchange(0);
		current.total_count.exchange(0);
...
static void sqlConnAddTimeInternal(SqlConn conn, long long dt_usec, long long idle_usec)
{
	...
	hrclock::duration::rep cur_worst = sql.times[conn].worst_usec;
	while ((ULONG)dt_usec > cur_worst) {
		if (sql.times[conn].worst_usec.compare_exchange_strong(cur_worst, dt_usec))
		{
			cur_worst = dt_usec;
		}
	}
	...
	sql.times[conn].running_usec.exchange(dt_usec);
	sql.times[conn].idle_usec.exchange(dt_usec);
	++sql.times[conn].total_count;
}

static void sqlConnTimerAddTime(SqlConn conn, timer::microrep since_last)
{
	...
	timer::microrep dt = sql.timers[conn].reset_micro();
    sqlConnAddTimeInternal(conn, dt, since_last);
}
...
static SQLRETURN _sqlConnStmtExecDirectUtf8(HSTMT stmt, char const* str, int str_len, SqlConn conn)
{
    ...
    static wchar_t*wstrs[SQLCONN_MAX];
    ...
    dynArrayFit(cpp_reinterpret_cast(void**)(&wstrs[conn]), 2, &wstr_maxs[conn], str_len+1);

    // Do a UTF8 to UTF16 conversion
    str_len = MultiByteToWideChar(CP_UTF8, 0, str, str_len, wstrs[conn], str_len+1);

	timer::microrep since_last = sql.timers[conn].reset_micro();
...
{
    SQLRETURN ret;
	hrclock::time_point lastFailTime;
	boolean firstFailTime = true;

	constexpr hrclock::duration maxRetryTime = time_utils::fromSeconds<hrclock::duration>(MAX_RETRY_SECONDS);
    ...
    for (;;) {
		...
		if (!firstFailTime)
		{
			lastFailTime = hrclock::now();
			firstFailTime = false;
		}
		else if ((hrclock::now() - lastFailTime) > maxRetryTime)
		{
...
int sqlConnStmtBindParam(HSTMT stmt, int param, int paramtype, int ctype, int sqltype, size_t colsize, int decimals,
						void* data, size_t buflen, long* strlen) {
    SQLRETURN ret;    
...
int sqlConnStmtStartBindParamTableValued(HSTMT stmt, int param, int maxrows, wchar_t const* tabletype, long*numrows) {
    SQLRETURN ret;
...
int sqlConnStmtBindCol(HSTMT stmt, int col, int ctype, void* data, size_t buflen, long* strlen)
{
    SQLRETURN ret;
...
const char * sqlConnStmtGetData(HSTMT stmt, int param, int ctype, ssize_t *length, const char *original_command, SqlConn conn)
{
