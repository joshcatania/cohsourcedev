...
TokenizerParseInfo ParseNamedTable[] = {
    {"{", TOK_START, 0}, {"Name", TOK_STRING(NamedTable, pchName, 0)}, {"Values", TOK_F32ARRAY(NamedTable, pfValues)}, {"}", TOK_END, 0}, {"", 0, 0}};
#endif
...
typedef struct CharacterClass
{
    ...
    bool bIsPetClass;
    // Does this class have access to bodyguarding/advanced MM-style
    // pets? Defaults to false.
    // Replaces most hardcoded Mastermind references.
    // IF YOU USE THIS BRANCH, ADD "IsPetClass  kTrue" TO
    // THE MASTERMIND'S CLASS DEF. v_pc_mastermind.def

    float fGuardShare;
    // What size is this AT's damage share when bodyguarding is active?
    // Defaults to 2.0f (based on the Mastermind's bodyguard share).
...
TokenizerParseInfo ParseCharacterClass[] = {{"{", TOK_START, 0},
...
#ifdef CLASSES_PARSE_INFO_DEFINITIONS
TokenizerParseInfo ParseCharacterClasses[] = {{"Class", TOK_STRUCT(CharacterClasses, ppClasses, ParseCharacterClass)}, {"", 0, 0}};
#endif
