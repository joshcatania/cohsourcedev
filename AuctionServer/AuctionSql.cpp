...
bool aucsql_insert_shard(const char** estr_shard)
{
	...
	SQLINTEGER shard_length = estrLength(estr_shard);
...
bool aucsql_insert_or_update_ent(int ent_id, const char* shard, const char** estr_ent)
{
	...
	SQLINTEGER shard_length = strlen(shard);
	SQLINTEGER ent_length = estrLength(estr_ent);
...
bool aucsql_insert_or_update_history(const char* identifier, const char** estr_history)
{
	...
	SQLINTEGER idenitifer_length = strlen(identifier);
	SQLINTEGER history_length = estrLength(estr_history);
