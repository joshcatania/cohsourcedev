...
static int compareEventHistoryEntry(const void * dataA, const void * dataB)
{
	const EventHistoryEntry** a = (const EventHistoryEntry**)dataA;
	const EventHistoryEntry** b = (const EventHistoryEntry**)dataB;
...
static int compareArenaRankingTableEntry(const void* dataA, const void* dataB)
{
	const ArenaRankingTableEntry** a = (const ArenaRankingTableEntry **)dataA;
	const ArenaRankingTableEntry** b = (const ArenaRankingTableEntry **)dataB;
...
{                                                                                    \
    ...
    \
    if(downpairing)                                                                    \
    eaSwap((cEArrayHandle*)(&sides), pairidx + groupsize/2, i);                                \
        else                                                                            \
        eaSwap((cEArrayHandle*)(&sides), pairidx - groupsize/2, i);                                \
        \
                    ...
                    int pairidx, int groupsize, int downpairing, int mediangroup)
{
    ...
    if(!valid_opponent && !mediangroup && !sides[i]->haveswitched)    // volunteer to float
    {
        if(downpairing)
        {
            ...
            eaSwap((cEArrayHandle*)&sides, pairidx, startidx + groupsize - 1);
...
void EventCreatePairingsSwissDraw(ArenaEvent* event)
{
    ...
    {
        event->roundtype[event->round] = (ArenaRoundType)1;
        ...
        if(event->round > 1)
        {
            ...
            for(i = 0; sides[i]->group < numgroups / 2; i++)
            {
                ...
                if(!haveopponent && sides[i]->roundlastfloated != (event->round - 1))
                {
                    ...
                    eaSwap((cEArrayHandle*)&sides, i, groupstart + groupsize - 1);
                    groupsize--;
                    i = groupstart - 1;
                    numdownfloats++;
                    continue;
            ...
            for(; i < eaSize(&sides) && sides[i]->group == mediangroup; i++)
            {
                ...
                if(!haveopponent)
                {
                    ...
                    eaMove((cEArrayHandle*)&sides, medianstart + groupsize-1, j);
                    eaMove((cEArrayHandle*)&sides, medianstart + groupsize, k);
                    if(j < medianstart)
                    {
                        medianstart -= 2;
                        i -= 2;
...
static int compareLeaderBoardEntry(const void* dataA, const void* dataB)
{
	const ArenaRankingTableEntry** a = (const ArenaRankingTableEntry**)dataA;
	const ArenaRankingTableEntry** b = (const ArenaRankingTableEntry**)dataB;
