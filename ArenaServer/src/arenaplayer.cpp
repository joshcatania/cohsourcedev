...
ArenaPlayer* PlayerGet(U32 dbid, int req_new)
{
    ...
    if (!g_players)
        eaCreate(&g_players);
    if (!g_playerHash)
        g_playerHash = stashTableCreateWithStringKeys(100, (StashTableMode)(StashDeepCopyKeys | StashCaseSensitive) );

    sprintf(id, "%i", dbid);
    ap = (ArenaPlayer*)stashFindPointerReturnPointer(g_playerHash, id);
