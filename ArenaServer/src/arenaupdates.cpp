...
{    
    ...
    sprintf(id, "%i", dbid);
    if (!g_dbidhashes)
        g_dbidhashes = stashTableCreateWithStringKeys(1000, (StashTableMode)(StashDeepCopyKeys | StashCaseSensitive) );
    stashFindPointer( g_dbidhashes, id, cpp_reinterpret_cast(void**)(&list));
    if (!list)
    {
...
{
    ArenaClientLink* client = cpp_reinterpret_cast(ArenaClientLink*)(link->userData);
    client->link = link;
    link->cookieEcho = 1;
...
void SetPlayerTTL(U32 dbid, int clear)
{
    ...
    if (!dbid)
        return;
    if (!g_ttlhashes)
        g_ttlhashes = stashTableCreateWithStringKeys(1000, (StashTableMode)(StashDeepCopyKeys | StashCaseSensitive) );
    sprintf(id, "%i", dbid);
    stashFindPointer( g_ttlhashes, id, cpp_reinterpret_cast(void**)(&rec) );
    if (clear)
    {
        if (!rec)
            return;
...
{
	DbidTTL* rec = cpp_reinterpret_cast(DbidTTL*)(stashElementGetPointer(el));
