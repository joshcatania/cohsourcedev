...
void baseedit_MoveDetail(BaseRoom *pRoom, RoomDetail *pDetail, Mat4 mat);
    // Moves the given detail in the given room to the given location.

void baseedit_MoveDetailToRoom(BaseRoom *pRoom, RoomDetail *pDetail, BaseRoom *pNewRoom, Mat4 mat);
...
bool baseedit_IsDoorway(BaseRoom *pRoom, int block[2], F32 height[2]);
    // Returns true if the given block is a doorway. If height is not NULL,
    //   the floor and ceiling heights for the doorway are returned.
...
static INLINEDBG void baseedit_MoveCurDetail(Mat4 mat)
        { if(g_refCurRoom.p && g_refCurDetail.p) baseedit_MoveDetail(g_refCurRoom.p, g_refCurDetail.p, mat); }
