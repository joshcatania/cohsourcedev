...
static const BasePower* pPower2;
...
static HybridElement powerTabs[] = {
    {0, "PrimaryPowerString", 0, "icon_power_primary_0", "icon_power_primary_1", 0, "powertab_tab_primary_0", "powertab_tab_primary_1",
     "powertab_tab_primary_2"},
    {1, "SecondaryPowerString", 0, "icon_power_secondary_0", "icon_power_secondary_1", 0, "powertab_tab_secondary_0", "powertab_tab_secondary_1",
     "powertab_tab_secondary_2"},
};
...
static int primaryChosen(int foo)
{
    return !!g_pPrimaryPowerSet;
}
static int secondaryChosen(int foo)
{
    return !!g_pSecondaryPowerSet && !!g_pSecondaryPowerSetValid;
}
static int powerChosen(int foo)
{
    return !!pPower1;
}
...
void powerMenuExitCode()
{
    ...
    if (pset && !character_OwnsPower(e->pchar, pPower2))
    {
        character_BuyPower(e->pchar, pset, pPower2, 0); // no choice, buy first power
    }
    sPrimaryPowersetChanged = 0;
...
static void initPowerSetSelector(const CharacterClass* pClass, int isSecondary)
{
    ...
    for (i = 0; i < eaSize(&pClass->pcat[isSecondary]->ppPowerSets); ++i)
    {
        ...
        {
            pElem->storeLocked = !accountEval(inventoryClient_GetAcctInventorySet(), db_info.loyaltyStatus, db_info.loyaltyPointsEarned,
                                              (U32*)db_info.auth_user_data, pSet->pchAccountRequires);
        }
...
static void refreshPowerSetSelector()
{
    ...
    for (i = eaSize(&ppPowerSetList) - 1; i >= 0; --i)
    {
        ...
        {
            ppPowerSetList[i]->storeLocked = !accountEval(inventoryClient_GetAcctInventorySet(), db_info.loyaltyStatus, db_info.loyaltyPointsEarned,
                                                          (U32*)db_info.auth_user_data, pSet->pchAccountRequires);
        }
    }
}
...
static int updateAndDrawPowerSets(F32 topX, F32 topY, F32 z, F32 powersetAlpha, bool* isPowerSetGray)
{
    ...
    for (i = 0; i < eaSize(&ppPowerSetList); ++i)
    {
        ...
        if ((eaSize(&ppPowerSetList) > NUM_POWERS_SHOWING) &&
            ((drawUp && (190 + 42 * i + yPowerSetOffset) < (190 + 42 * 1.5f)) || (drawDn && (190 + 42 * i + yPowerSetOffset) > (190 + 42 * 6.5f))))
        {
            alpha = 0.25f;
        }

        barRet = drawHybridBar(ppPowerSetList[i], selected, midX, (190 + 42 * i + yPowerSetOffset) * yposSc, z, 450.f, 1.f, flags, alpha, H_ALIGN_LEFT,
                               V_ALIGN_CENTER);
        ...
        if (ppPowerSetList[i]->storeLocked)
        {
            ...
            // draw store icon
            drawHybridStoreMenuButton(pBPset->pchAccountProduct, midX + 185.f * xposSc, (190 + 42 * i + yPowerSetOffset) * yposSc, z, 0.6f, CLR_WHITE,
                                      HB_STORE_PRODUCT);
        }
    }
...
static int updateAndDrawPowers(F32 topX, F32 topY, F32 z, F32 powerAlpha, bool isPowerSetGray)
{
    ...
    if (sCurrentPowerSet >= 0 && prev_selected == NULL)
    {
        ...
        else
        {
            ...
            if (!sSwitchedSet)
            {
                pPower2 = 0; // reset chosen power since the powerset just changed
            }
    ...
    for (i = 0; i < eaSize(&ppPowerList); i++)
    {
        ...
        if (!secondarySelected)
        {
            selected = (pPower1 == ppPowerList[i]->pData);
        }
        else
        {
            selected = pPower2 == ppPowerList[i]->pData;
        }

        F32 alpha = 1.f;
        int barRet;

        bool canIhavePower = character_IsAllowedToHavePowerHypothetically(playerPtr()->pchar, ppPowerList[i]->pData);
        ...
        if ((eaSize(&ppPowerList) > NUM_POWERS_SHOWING) &&
            ((drawUp && (190 + 42 * i + yPowerOffset) < (190 + 42 * 1.5f)) || (drawDn && (190 + 42 * i + yPowerOffset) > (190 + 42 * 6.5f))))
        {
            alpha = 0.25f;
        }

        barRet =
            drawHybridBar(ppPowerList[i], selected, midX, (190 + 42 * i + yPowerOffset) * yposSc, z, 450.f, 1.f, flags, alpha, H_ALIGN_LEFT, V_ALIGN_CENTER);
        if (D_MOUSEHIT == barRet)
        {
            if (!(flags & HB_UNSELECTABLE)) // secondary power is already chosen
            {
                if (!secondarySelected)
                {
...
void powerMenu()
{
    ...
    menuHybridDisplayPowerInfo(xp - 462.f * xposSc, 150.f * yposSc, 10, 500, 525,
                               (eaSize(&ppPowerList) && lastHoveredPower >= 0) ? ppPowerList[lastHoveredPower]->pData : 0, 0, POWERINFO_HYBRID_BASE,
                               detailScale);
    if (showDetailedDesc)
    {
    ...
    // tab text
    {
        int alphafont[2] = {0xffffff00 | (int)(50.f + powerTabs[0].percent * 205.f), 0xffffff00 | (int)(50.f + powerTabs[1].percent * 205.f)};
