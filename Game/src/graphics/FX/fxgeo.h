...
typedef struct FxMiniTracker 
{
    ...
    int count;
    FxHandle fxIds[MAX_FXMINITRACKERS];
    U8 alpha;
} FxMiniTracker;
...
typedef struct FxGeo
{
    ...
    FxHandle            magnetid;                  // handle of magnet fxgeo
    FxHandle            lookatid;                  // handle of "other" fxgeo
    FxHandle            pmagnetid;                 // handle of magnet for particle systems fxgeo
    FxHandle            potherid;                  // handle of pother for particle systems fxgeo
...
FxHandle fxGeoCreate(void** fxgeolist, int* fxgeocount, GfxNode* key, Mat4 offset, FxEvent* fxevent, int iskey, FxInfo* fxInfo, DefTracker* light);
...
int fxGeoAttachChildFx(FxGeo* fxgeo, char* childfx, FxParams* parentFxp, FxHandle seq_handle, int target_seq_handle);
