...
typedef struct FxParams
{
    ...
    FxHandle    targetSeqHandle; // Stored separately, since it might be needed by a child effect.
    FxHandle    prevTargetSeqHandle;
...
typedef struct FxObject
{
    ...
    FxHandle handle_of_parent_seq; // new crazy concept, knowing about the seq you are attached to...
    FxHandle handle_of_target_seq; // if a target was passed in as a key, this is it's seq (for ChildFx)
    FxHandle last_move_of_parent;
...
FxHandle fxCreate(const char* fxname, FxParams* fxp);
...
void fxResetStaticLighting(FxHandle fxid); // clears the lighting on all fxgeos so that it will be recalculated next frame
