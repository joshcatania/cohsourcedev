...
static int mapping0_forward(vorbis_block *vb){
  ...
  for(i=0;i<vi->channels;i++){
    ...
    scale_dB= (float)(todB(&scale) + .345); /* + .345 is a hack; the original
    ...
    logfft[0] = (float)(scale_dB+todB(pcm)  + .345); /* + .345 is a hack; the
    ...
    for(j=1;j<n-1;j+=2){
      ...
      temp= logfft[(j+1) >> 1] = (float)(scale_dB+.5f*todB(&temp)  + .345); /* +
  ...
  
  {
    ...
    for(i=0;i<vi->channels;i++){
     ...
     specific bitrate tweaking use the same floor */
      ...
      for(j=0;j<n/2;j++)
    logmdct[j]= (float)(todB(mdct+j)  + .345); /* + .345 is a hack; the original
                                     todB estimation used on IEEE 754
                                     compliant machines had a bug that
                                     returned dB values about a third
                                     of a decibel too high.  The bug
