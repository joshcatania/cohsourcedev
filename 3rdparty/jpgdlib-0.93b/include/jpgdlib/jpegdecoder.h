...
public:
  ...
  bool set_mem_ptr(const char *mem,int size)
  {
    curr_idx = 0;
    mem_size = size;
    Pfile = mem;
  ...
  virtual int read(uchar *Pbuf, int max_bytes_to_read, bool *Peof_flag)
  {
    ...
    int bytes_read,left_to_read = mem_size-curr_idx;
    if (left_to_read < max_bytes_to_read)
    {
    ...
    memcpy(Pbuf,Pfile+curr_idx,bytes_read);
    curr_idx += bytes_read;
