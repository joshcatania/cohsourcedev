...
// #line 371 "vs1.0_tokens.l"
{
    ...
    offset = (unsigned int)strcspn(yytext, " \t\n_");
    yyless(offset);
...
//    GenListString();
    ...
    if ((yytext[0] == '"') || (yytext[0] == '<'))
    {
        ...
        sLen = (unsigned long)((endQuote - yytext) - 1);
        if (endQuote == NULL)
        {
    ...
    if (validFileName)
    {
        sLen = (unsigned long)strlen(incFileName);
...
// #line 600 "vs1.0_tokens.l"
{
    ...
    if (endComment != NULL)
    {
        keepSize = (unsigned int)((endComment - yytext + 2));
        yyless(keepSize);
        BEGIN(INITIAL);
...
//    fprintf( stderr, "%s", yytext );
    ...
    int validLen = (int)strspn(yytext, "xyzw");
        int i;
...
{ 
    ...
    startOffset = (unsigned int)strspn(yytext, " \t");
...
// #line 1311 "vs1.0_tokens.l"
{
    ...
    copyLen = (unsigned int)(endLine - yytext);
...
void CheckMacroFunctions(char *lookString, unsigned int *recognizedLen, char **invString)
{
    ...
    {
        sLen = (unsigned int)strlen(gMacroFunctions[i].name);
                        ...
                        char *lookString, unsigned int *recognizedLen, char **invString)
{
    ...
    *invString = NULL;
    ...
    while (checkStr != NULL)
    {
        ...
        while (srcText)
        {
            sLen = (unsigned int)strlen(srcText->macroText);
    ...
    if (srcText != NULL)
    {
        ...
        while (srcText != NULL)
        {
            ...
            if (!strncmp(srcText->macroText, lookString, sLen))
            {
                ...
                *recognizedLen = (unsigned int)strlen(srcText->macroText);
                *invString = invText->macroText;
                            ...
                            MACROENTRY *srcParms, MACROENTRY *invParms)
{
    ...
    sLen = (unsigned int)strlen(srcLine);
    dLen = 0;
    ...
    while (sLen > 0)
    {
        ...
        if (!srcParms->bIsDefine)
        {
            ...
            if (findReplace != NULL)
            {
                ...
                copyLen = (unsigned int)((findReplace - srcLine) - 1);
        ...
        else
        {
            ...
            {
                // figure out length of source before %
                copyLen = (unsigned int)(findReplace - srcLine);
            }
            else
            {
                strcat(destLine, srcLine);
        ...
        if (invString != NULL)
        {
            ...
            subLen = (unsigned int)strlen(invString);
        }
        else
        {
            subLen = 0;
        ...
        if (invString != NULL)
        {
            ...
            dLen += (unsigned int)strlen(invString);
        }
                        ...
                        char *mathStr)
{
    ...
    if (numStartStr != NULL)
    {
        ...
        {
            sLen = (unsigned int)(numStartStr - *invStr);
            gReplaceText[0] = '\0';
...
void MacroAddFunction(char *lookStr, unsigned int *recognizedLen, char **invStr)
{
    ...
    if (ParseBuiltInMacroParms(&tMEntry, lookStr))
    {
        MacroMathFunction(&tMEntry, recognizedLen, invStr, "+");
        // skip ',' strlen(parm2)+ ')'
        (*recognizedLen) += (unsigned int)strlen(tMEntry.firstMacroParms->next->macroText)+2;
    }
...
unsigned int FindSwizzleValue(char *swizzleText)
{
    ...
    sLen = (unsigned int)strlen(swizzleText);
    swizzleBits = 0;
    lastMask = 0;
...
// Description:    Look through mask strings
// Parameters:    findName = name to lookup
