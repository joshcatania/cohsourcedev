...
namespace
{
    ...
    struct src
    {
        ...
        void init(string s, int stage, string *regname=NULL)
        {
            ...
            if (regname != NULL) 
                *regname = s; // return the bare name
    ...
    struct set_register_combiners
    {
        ...
        void operator() (vector<string> & instr)
        {
            ...
            if(op == "add" || op == "sub")
            {
                src a(instr[2],combiner);
                src b(instr[3],combiner);
            ...
            {
                src a(instr[3],combiner);
                src b(instr[4],combiner);

                if(instr[2] != "r0.a" && instr[2] != "r0.w")
                {} // bad
                if(mask == GL_RGBA || mask == GL_RGB)
                {
                    glCombinerInputNV(C, GL_RGB, GL_VARIABLE_A_NV, a.reg, a.map, a.comp);
            ...
            else if(op == "dp3")
            {
                ...
                if(mask == GL_RGBA || mask == GL_ALPHA)
                {
                    ...
                    // Done by Ashu: Put this in the alphaBlueRegister set. 
                    isAlphaBlue = true;
                  ...
                  list<vector<string> > * b)
{
    ...
    glCombinerParameteriNV(GL_NUM_GENERAL_COMBINERS_NV, (GLint)b->size());
    if(b)
...
bool ps10::init_extensions()
{
    ...
    if (tsinit == false )
    {
        if(! glewIsSupported( "GL_NV_texture_shader " "GL_ARB_multitexture " ))
        {
...
const int* ps10_get_info(int* pcount)
{
    if (pcount)
    {
        *pcount = (int)constToStageArray.size();
    }
