...
BOOL CALLBACK EnumProcCountCrashedMapservers(HWND hwnd, LPARAM lParam)
{
    ...
    if(GetWindowThreadProcessId(hwnd, &processID))
    {
        if(ProcessNameMatch(processID, "mapserver.exe"))
        {
            ...
            if(GetWindowTextA(hwnd, title, MAX_WINDOW_TITLE_SIZE))
            {
...
BOOL onUpdateSelf(Packet * pak)
{
    ...
    if(file)
    {
        ...
        res = execAndQuit(oldFilename, GetCommandLineA());    // will terminate inside this function on success
        if(!res)
...
BOOL onApplyPatch(char * updateSvr)
{
    ...
    // -------------------------------------------------
    {
        ...
        if( !CreateProcessA(filename, // No module name (use command line). 
...
BOOL StartProgram(const char * filename, const char * params)
{
    ...
    ret = 32 < (intptr_t)ShellExecuteA(0, "open", actualFilename, params, g_workingDir[0] ? g_workingDir : regGetInstallationDir(), SW_SHOW);
...
BOOL startProcessAndWait(char * exeName, char * params)
{
    ...
    // Start the child process. 
    if( !CreateProcessA(exeName, // No module name (use command line). 
        params, // Command line. 
        NULL,             // Process handle not inheritable. 
        NULL,             // Thread handle not inheritable. 
        FALSE,            // Set handle inheritance to FALSE. 
