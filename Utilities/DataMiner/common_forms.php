...
// hacked to make training room the default
function select_server($name = '', $default = false)
{
...
function select_frequency($name = '', $default = COMMON_DEF_FREQUENCY)
{
    ...
    $r = "<select name='frequency_$name' id='$name.f' onChange='frequency_changed(\"$name\");'>";
...
function form_mining_prolog()
{
    ...
    //  comparator_changed(asel)
    $r .= "function comparator_changed(asel) {\n";
    $r .= "	var elems = asel.parentNode.childNodes;\n";
    $r .= "	var dsel;\n";
    $r .= "	for(var i = 0; i < elems.length; i++)\n";
    $r .= "		if(elems[i].name.substr(0,9) == 'whereval_') {\n";
    $r .= "			dsel = elems[i];\n";
    $r .= "			break;\n";
    $r .= "		}\n";
    $a = array();
    foreach ($common_comparison as $k => $v) {
        if (@$v['nodata']) {
            $a[] = "asel.options[asel.selectedIndex].value == '$k'";
        }
    }
    ...
    // frequency_changed
    //    $r .= "function frequency_changed(name) {\n";
    //    $r .= "    var csel = document.getElementById(name+'.f');\n";
    //    $r .= "    var frequency = csel.options[csel.selectedIndex].value;\n";
    //    $r .= "    document.getElementById(name+'.email').style.display = (frequency != 0 ? 'block' : 'none');\n";
    //    //$r .= " echo 'Blah'\n";
    //    $r .= "}\n";
    //    $r .= "\n";
    ...
    foreach ($templates as $container => $v) {
        global $common_email_address;

        // Email Line
        $r .= "<div id='emailline_${container}' style='display:none'>";
        ...
        $r .= "<select name='aggregation_' onChange='aggregator_changed(this);'>";
        ...
        $r .= "<optgroup label='Magic'><option value=0>Shard</optgroup>";
        foreach ($v as $tablename => $w) {
            $r2 = '';
            foreach ($w as $field) {
                if ($field['groupok']) {
...
function form_mining($name, $default_container = '', $default_field = '')
{
    ...
    $r .= "</select>\n";
    $r .= "</div>\n";
    $r .= "<div class='formrow'><div class='label'>Frequency</div>" . select_frequency("$name") . "</div>\n";
    $r .= "<div class='formrow' id='$name.t'><div class='label'>Time Frame</div>" . select_timeframe("timeframe_$name") . "</div>\n";

    $r .= "<div class='formrowlong' id='$name.email'><div class='label'><a href='javascript:' onClick='addline(\"$name\",\"email\");'><img src='img/listadd.png' title='Add Email' class='listicon'></a> Email Notification</div></div>\n";
