...
static void initUser(User *user, StashTable stash)
{
    ...
    // don't run the expiration code if we're in the process of converting from old to new gmail
    if (!converting)
    {
        for(i = eaSize(&gmuser->gmail)-1; i>=0; i--)
        {
            Email *email = gmuser->gmail[i];

            if(!email->body)
                email->body = StructAllocString("");
            if(!email->subj)
                email->subj = StructAllocString("");
            if(!email->attachment)
                email->attachment = StructAllocString("");

            if( now - email->sent  > GMAIL_EXPIRATION_DURATION )
            {
                if( email->attachment && *email->attachment ) 
                    bounceGmail(user, email, false ); // bouncing will delete email
                else
                    gmailRemove(gmuser, email->id);
                gmdirty = true;
                continue;
            }
        }
        ...
        for(i = eaSize(&gmuser->sent_mail)-1; i>=0; i--)
        {
            if( now - gmuser->sent_mail[i]->sent  > GMAIL_EXPIRATION_DURATION )
            {
                SentMail * pDel = eaRemove(&gmuser->sent_mail,i);
                if (pDel->attachment)
                    ParserFreeString(pDel->attachment);
                ParserFreeStruct(pDel);
                gmdirty = true;
                continue;
            }
        }
