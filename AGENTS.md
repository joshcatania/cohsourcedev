...
- A **native GLSL pilot** now renders `BLENDMODE_MODULATE`, `BLENDMODE_MULTIPLY`, and `BLENDMODE_COLORBLEND_DUAL` (the costume-tint material) through hand-written GLSL 1.20 compatibility-profile programs (`Game/src/render/thread/rt_glslpilot.{c,h}`), enabled with `-glslPilot 1` (or `agent/capture.ps1 -ExtraClientArgs "-glslPilot 1"`; `agent/capture-regression.ps1` forwards the same parameter for whole-suite runs). It is harness-verified visually equivalent to the ARB path: the full 4-shot suite is green with all active materials confirmed in the client log. The materials share one replicated `vp_master_vp.cg` vertex shader and differ only in fragment math; multiply consumes the mirrored `g_Env0FP` engine constant (`constColor0`, fragment `program.env[8]`), colorBlendDual both `g_Env0FP` and `g_Env1FP` (`constColor1`, `program.env[9]`). It is the proven template for porting further materials off Cg.
...
1. extend the GLSL pilot to further materials in small, harness-verified steps (modulate, multiply, and colorBlendDual are done; next: `addGlowfp`/`addglowfp.cg`, which consumes `g_GlowParamFP` — same mirror technique); bumpmapped materials additionally need tangent-space interpolants and more engine constants
