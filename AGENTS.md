...
- A **native GLSL pilot** renders `BLENDMODE_MODULATE`, `BLENDMODE_MULTIPLY`, `BLENDMODE_COLORBLEND_DUAL` (the costume-tint material), and `BLENDMODE_BUMPMAP_COLORBLEND_DUAL` (the bumped dual-tint material, variant 0) through hand-written GLSL 1.20 compatibility-profile programs (`Game/src/render/thread/rt_glslpilot.{c,h}`), enabled with `-glslPilot 1` (or `agent/capture.ps1 -ExtraClientArgs "-glslPilot 1"`; `agent/capture-regression.ps1` forwards the same parameter for whole-suite runs). It is harness-verified visually equivalent to the ARB path day and night: the full 4-shot suite is green with all active materials confirmed in the client log. The simple materials share one replicated `vp_master_vp.cg` dualtex vertex shader (mode-switched by uniform); the bump material has its own vertex shader covering both the static `bump_dual` variant and the skinned `skin_bump` variant (two-bone skinning through the mirrored bone-matrix array; on Atlas Park the bump-dual coverage is mostly the skinned player costume). Mirrored engine constants cover `g_ReflectionParamVP`, `g_Env0FP`/`g_Env1FP` (constColor0/1), `g_GlowParamFP`, and the bump set (`g_LightDirVP`, `g_AmbientColorFP`/`g_DiffuseColorFP`/`g_GlossParamFP`/`g_Specular1ColorAndExponentFP`, `g_BoneMatrixArrVP`). `BLENDMODE_ADDGLOW` and `BLENDMODE_ALPHADETAIL` are ported and registered but **unverified: no available Atlas Park view ever binds them** (night shots included — the capture shot table gained `timeHour` and Night* labels to prove that). The HQ bump variants (e.g. fragment 101) are not ported and render through ARB. Unported materials idle as safe fallbacks; with the pilot on, a per-process coverage diagnostic logs every unported fragment program id bound during a capture.
...
1. renderer material ports in small, harness-verified steps (modulate, multiply, colorBlendDual, bumpmapColorblendDual variant 0 verified; addGlow and alphaDetail ported but unverified — no Atlas Park coverage, per the coverage diagnostic). The remaining Atlas-Park-covered fragment families are the HQ bump variants (fragment 101: tangent/normal/position interpolants plus cubemap/shadow extensions), `bumpmapMultiply` (model-space lighting path), `multi9` (multi-material, the big one), water, and the effects/ post-processing set; the map-transfer capture path is the gate for verifying the uncovered materials and for scenes beyond Atlas Park
