...
1. renderer material ports in small, harness-verified steps (modulate, multiply, colorBlendDual done; addGlow ported but awaiting scene coverage): next is either the map-transfer capture path to reach AddGlow-covered scenes, or the remaining simple materials (`alphaDetailfp`, `waterfp`); bumpmapped materials additionally need tangent-space interpolants and more engine constants
