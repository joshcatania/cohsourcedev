...
1. renderer material ports in small, harness-verified steps (modulate, multiply, colorBlendDual, bumpmapColorblendDual variant 0 + HQ variant 101, the effects/ post-processing set, and `bumpmapMultiply` (fragment 84, the model-space water-fallback material) all verified; addGlow ported but unverified — no binding view known). Remaining material families are gated: `multi9` (multi-material, the big one) waits on a mission-instance capture path, and water waits on the `GFXF_MULTITEX` startup ordering fix; `FoundersCanal_01` (map 10, Founders Falls canals) is the non-Atlas regression shot covering bumpmapMultiply and alphaDetail
