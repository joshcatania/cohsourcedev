...
1. renderer material ports in small, harness-verified steps (modulate, multiply, colorBlendDual, bumpmapColorblendDual variant 0 + HQ variant 101 verified; addGlow and alphaDetail ported but unverified — no Atlas Park coverage, per the coverage diagnostic). The remaining Atlas-Park-covered fragment families are `bumpmapMultiply` (model-space lighting path; not yet observed bound in the Atlas shots), `multi9` (multi-material, the big one), water, and the effects/ post-processing set (fragment ids 201-216, the only unported ids still observed bound in an Atlas capture); the map-transfer capture path is the gate for verifying the uncovered materials and for scenes beyond Atlas Park
