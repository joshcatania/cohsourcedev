...
- Water surfaces on static maps deterministically bind bumpmapMultiply (fragment 84), not the fancy-water shader (fragment 118): `BLENDMODE_WATER` is assigned at texture-load time and the multi-texture branch needs `GFXF_MULTITEX`, which the `DRAWMODE_BUMPMAP_MULTITEX` vertex-program load ordering already cleared by the time map texopts bind. Pinning `waterMode` does not change this. A true water port is gated on fixing that startup ordering; multi9 (fragment 120) never bound across eleven probed zones including interiors and is likely mission-instance-only, gated on a mission-instance capture path.
...
1. renderer material ports in small, harness-verified steps (modulate, multiply, colorBlendDual, bumpmapColorblendDual variant 0 + HQ variant 101, and the effects/ post-processing set all verified; addGlow and alphaDetail ported; alphaDetail now has deterministic coverage on the `FoundersCanal_01` shot). The map-transfer capture path is done and verified — `FoundersCanal_01` (map 10, Founders Falls canals) is the first non-Atlas regression shot and deterministically binds `bumpmapMultiply` (fragment 84, the water texopt auto-fallback) plus alphaDetail, at 1.08% drift. Next material: `bumpmapMultiply` (model-space lighting path, verify on `FoundersCanal_01`); after that `multi9` (multi-material, the big one) waits on a mission-instance capture path and water waits on the `GFXF_MULTITEX` startup ordering fix
