...
$evidenceRoot = Join-Path $repoRoot 'docs/evidence/issue26-atlas-remaster'
...
try {
    ...
    $extraArgs = "-glslPilot $GlslPilot -modernMaterials 1 -modernPresentation 0 -modernBloom 0 -modernLighting $modernLightingValue"
    if ($LightDebug -ge 0) { $extraArgs += " -lightdebug $LightDebug" }
    $captureArgs = @(
...
if ($Json) {
    ...
    Write-Host "LIGHTING PILOT PASS - $imageOutputPath"
} else {
    Write-Host "LIGHTING PILOT FAIL - $($result.reason)"
}
