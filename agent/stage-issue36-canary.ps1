...
$generated = $false
if ($Variant -eq 'FullFrame') {
    if ($FrameStart -ge 60) { throw 'FullFrame requires FrameStart 1..59 so the Anim range has an end frame.' }
    $endFrame = $FrameStart + 1
    $content = @"
// Issue 36 forensic canary: one frozen source frame from the proven full clip.
...
Move COHSOURCEDEV_CUSTOM_CANARY
	...
	Scale 0
	Type Male
		Anim MALE/COHSOURCEDEV_RETARGET_SWING_FULL $FrameStart $endFrame
		TEnd
