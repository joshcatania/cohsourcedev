...
countFame = function(fame, increase)
	--local fame = 0
	--local increase = 25
	if Mission.ObjectiveIsSucceeded("Bomb") then
		fame = fame + 100
	end
	if Mission.ObjectiveIsSucceeded("Carnival1") then
		fame = fame + increase
	end
	if Mission.ObjectiveIsSucceeded("Carnival2") then
		fame = fame + increase
	end
	if Mission.ObjectiveIsSucceeded("Carnival3") then
		fame = fame + increase
	end
	if Mission.ObjectiveIsSucceeded("Doppelganger") then
		fame = fame + increase
	end
	return fame
end
				...
				UICollection:new
				{
					...
					items =
					{
						UIItem:new
						{
							...
							currentVal = function() return countFame(100, 40) end,
							...
							targetVal = function() return 300 end
						}
					}
				}
			}
