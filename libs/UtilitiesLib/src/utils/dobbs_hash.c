//
// A hash function by Bob Jenkins taken from Dr. Dobbs.  Copyright free.
//
//    http://ourworld.compuserve.com/homepages/bob_jenkins/evahash.htm
//
// Sept. 1997 Dr. Dobbs, page 115.
//
#if 0
#include <string.h>
...
u4 hashCalc( const void *k_void, u4 length, u4 initval )
{
...
u4 str_hash(const u1 *s)
{
    ...
    strcpy(buf,s);
    strupr(buf);
    return hashCalc(buf, strlen( buf ), DEFAULT_HASH_SEED);
}
